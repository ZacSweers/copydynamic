...
import io.sweers.metric.readKotlinClassMetadata
...
class CopyDynamicProcessor : AbstractProcessor() {
  ...
  private fun createType(element: TypeElement,
    ...
    val kmClass = (element.getAnnotation(Metadata::class.java).readKotlinClassMetadata() as KotlinClassMetadata.Class).toKmClass()
    val signature = kmClass.constructors.find { Flag.Constructor.IS_PRIMARY.invoke(it.flags) }!!.signature!!
    generateClass(packageName, "${builderName}Bridge", element.simpleName.toString(), filer, signature.desc, builderSpec.originatingElements)
